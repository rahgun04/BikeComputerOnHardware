...
void disp_flush_wrapper(lv_disp_drv_t* drv, const lv_area_t* area, lv_color_t* color_p) {
    
    fd.drv1 = drv;
    fd.area1 = area;
    fd.color_p1 = color_p;
    fd.syncBit = !fd.syncBit;


}

PI_THREAD(flush_thread) {
    bool state = fd.syncBit;
    while (1) {
        if (fd.syncBit != state) {
            state = fd.syncBit;
            ili9341_flush(fd.drv1, fd.area1, fd.color_p1);
        }
...
void disp_thread_start() {
    int x = piThreadCreate(flush_thread);
    if (x != 0) {
